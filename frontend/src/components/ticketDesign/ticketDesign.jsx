...
const getFlightLabel = (flight) => {
  if (flight?.id != null) return `#${flight.id}`;
  return "-";
};

const TicketDesign = ({ ticket = {}, flight = {}, passengerName }) => {
  ...
  const departureCountry = formatLabel(originAirport?.country?.name, "Origin");
  const destinationCountry = formatLabel(
    destinationAirport?.country?.name,
    "Destination"
  );
  const departureCode = formatLabel(
    originAirport?.iata_code ?? originAirport?.icao_code
  );
  const destinationCode = formatLabel(
    destinationAirport?.iata_code ?? destinationAirport?.icao_code
  );
  const departureHour = formatTime(flight.departure_time);
  const arrivalHour = formatTime(flight.landing_time);

  const ticketId = formatLabel(ticket.id, "-");
  const passenger = formatLabel(passengerName, "Passenger");
  const flightLabel = getFlightLabel(flight);
  const gateLabel = formatLabel(
    flight?.gate ?? flight?.gate_no ?? flight?.gate_number,
    "-"
  );
  const seatLabel = formatLabel(ticket.seat_no, "-");
            ...
            <div className="content-data">
              <div className="destination">
                <div className="dest start">
                  <p className="country">{departureCountry}</p>
                  <p className="acronym">{departureCode}</p>
                  <p className="hour">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={10}
                ...
                <div className="dest end">
                  <p className="country">{destinationCountry}</p>
                  <p className="acronym">{destinationCode}</p>
                  <p className="hour">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={10}
              ...
              <div className="data-flex-col">
                <div className="data-flex top-row">
                  <div className="data">
                    <p className="title">ID</p>
                    <p className="subtitle">{ticketId}</p>
                  </div>
                ...
                <div className="data-flex bottom-row">
                  <div className="data">
                    <p className="title">Flight</p>
                    <p className="subtitle">{flightLabel}</p>
                  </div>
                  <div className="data">
                    <p className="title">Gate</p>
                    <p className="subtitle">{gateLabel}</p>
                  </div>
                  <div className="data">
                    <p className="title">Seat</p>
                    <p className="subtitle">{seatLabel}</p>
                  </div>
                </div>
              </div>
            </div>
...
const StyledWrapper = styled.div`
  ...
  .content-data {
    ...
    & .bottom-row .data:nth-child(1) {
      text-align: left;
    }

    & .bottom-row .data:nth-child(2) {
      text-align: center;
    }

    & .bottom-row .data:nth-child(3) {
      text-align: right;
    }
  }
